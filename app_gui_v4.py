...
from convert_nexus_folder import convert_nexus_folder, convert_nexus_folder_flexpes
...
class BEAMTIMEBUDDEH:
    ...
    def setup_conversion_section(self):
        ...
        tk.Label(self.left_frame, text="i09", font=("Arial", 10, "bold")).pack()

        i09_frame = tk.Frame(self.left_frame)
        i09_frame.pack(fill="x", pady=(0, 10))
        ...
        tk.Button(
            i09_frame,
            text="Create Labbook",
            command=self.create_labbook_i09_gui 
        ).pack(side="left", expand=True, fill="x", padx=2)
        ...
        tk.Label(self.left_frame, text="FLEXPES", font=("Arial", 10, "bold")).pack()

        flexpes_frame = tk.Frame(self.left_frame)
        flexpes_frame.pack(fill="x")

        tk.Button(
            flexpes_frame,
            text="Convert Data",
            command=self.run_conversion_flexpes
        ).pack(side="left", expand=True, fill="x", padx=2)

        tk.Button(
            flexpes_frame,
            text="Create Labbook",
            command=self.create_labbook_flexpes_gui   
        ).pack(side="left", expand=True, fill="x", padx=2)
    ...
    def run_conversion(self):
        ...
        convert_nexus_folder(self.input_dir, self.output_dir)
    ...
    def run_conversion_flexpes(self):
        ...
        convert_nexus_folder_flexpes(self.input_dir, self.output_dir)
        messagebox.showinfo("Done", "Conversion complete.")
    def create_labbook_i09_gui(self):
        ...
        try:
            create_labbook_i09(self.input_dir,self.output_dir)
            messagebox.showinfo("Success", " Excel sheet produced by hardworking script goblins: ლ༼ ಥ 益 ಥ ༽ლ")
        except Exception as e:
            messagebox.showerror("Error", f"Labbook creation failed:\n{e}")
    def create_labbook_flexpes_gui(self):
        ...
        try:
            create_labbook_flexpes(self.input_dir,self.output_dir)
            messagebox.showinfo("Success", " Excel sheet produced by hardworking script goblins: ლ༼ ಥ 益 ಥ ༽ლ")
        except Exception as e:
            messagebox.showerror("Error", f"Labbook creation failed:\n{e}")
    # ==============================
    ...
    def setup_loading_section(self):
        ...
        tk.Button(self.left_frame, text="Select Spectra Folder",
    ...
    def setup_spectra_scroll_panel(self):
        ...
        self.spectra_vars = {}
    def create_labbook_i09_button(self):

        if not self.input_folder:
            messagebox.showwarning("No Input Folder", "Please select an input folder first.")
            return

        create_labbook_i09(self.input_folder)
        messagebox.showinfo("Success", " Excel sheet produced by hardworking script goblins: ლ༼ ಥ 益 ಥ ༽ლ")
    def create_labbook_flexpes_button(self):

        if not self.input_folder:
            messagebox.showwarning("No Input Folder", "Please select an input folder first.")
            return

        create_labbook_flexpes(self.input_folder)
        messagebox.showinfo("Success", " Excel sheet produced by hardworking script goblins: ლ༼ ಥ 益 ಥ ༽ლ")
    ...
    def load_spectra(self):
        ...
        new_data = load_spectra_from_folder(folder)
        ...
        if added == 0:
            messagebox.showinfo(
                "No New Spectra",
                "All loaded spectra already exist in the list."
            )
        else:
            messagebox.showinfo(
                "Spectra Loaded",
